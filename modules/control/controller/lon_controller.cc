...
Status LonController::Init(const ControlConf *control_conf) {
  control_conf_ = control_conf;       //ControlConf消息，在control_conf.proto文件中定义
  ...
  const LonControllerConf &lon_controller_conf =        //LonControllerConf消息，在lon_controller_conf.proto文件中定义
      control_conf_->lon_controller_conf();

  station_pid_controller_.Init(lon_controller_conf.station_pid_conf());   //根据lon_controller_conf配置文件初始化位置pid控制器
  speed_pid_controller_.Init(lon_controller_conf.low_speed_pid_conf());   //根据lon_controller_conf配置文件初始化速度pid控制器
...
void LonController::LoadControlCalibrationTable(
    const LonControllerConf &lon_controller_conf) {
  const auto &control_table = lon_controller_conf.calibration_table();    //获取control_conf.pb.txt文件中的校准表
  ...
  CHECK(control_interpolation_->Init(xyz))            //将校准表中的信息保存到control_interpolation_
      << "Fail to load control calibration table";
}

/**
   * @brief 根据当前车辆状态和目标轨迹计算刹车/油门值
...
Status LonController::ComputeControlCommand(
  ...
  ComputeLongitudinalErrors(trajectory_analyzer_.get(), preview_time, debug); //计算纵向偏差，station_error位置偏差和speed_error速度偏差

  double station_error_limit = lon_controller_conf.station_error_limit();
  double station_error_limited = 0.0;
  if (FLAGS_enable_speed_station_preview) {   //默认 enable_speed_station_preview = true
    station_error_limited =
    ...
    station_error_limited = common::math::Clamp(
        debug->station_error(), -station_error_limit, station_error_limit);   //station_error为位置误差
  }
  ...
  double speed_offset =
      station_pid_controller_.Control(station_error_limited, ts);   //计算速度补偿量speed_offset，使用位置pid控制器计算

  double speed_controller_input = 0.0;      //“速度-加速度控制器”的速度输入
  double speed_controller_input_limit =
      lon_controller_conf.speed_controller_input_limit();     //speed_controller_input_limit = 2.0
  double speed_controller_input_limited = 0.0;
  if (FLAGS_enable_speed_station_preview) {       //默认 enable_speed_station_preview = true
    speed_controller_input = speed_offset + debug->preview_speed_error();
  } else {
    speed_controller_input = speed_offset + debug->speed_error();   //speed_error为速度误差
  }
  speed_controller_input_limited =        //限定speed_controller_input在（-2.0，2.0）之间
      common::math::Clamp(speed_controller_input, -speed_controller_input_limit,
                          speed_controller_input_limit);

  double acceleration_cmd_closeloop = 0.0;    //加速度补偿量
  ...
  double acceleration_cmd =                 //标定表的加速度输入
  ...
  double calibration_value = 0.0;         //刹车/油门值
  ...
  debug->set_speed_lookup(chassis_->speed_mps());                   //当前车辆行驶速度，，以米每秒为单位
  debug->set_calibration_value(calibration_value);                  //刹车/油门值
...
void LonController::ComputeLongitudinalErrors(
  ...
  // the decomposed vehicle motion onto Frenet frame          将车辆运动分解到Frenet坐标系
  ...
  double s_matched = 0.0;         //沿参考轨迹的纵向累积距离
  double s_dot_matched = 0.0;     //沿参考轨迹的纵向速度
  double d_matched = 0.0;         //横向距离w.r.t.参考轨迹
  double d_dot_matched = 0.0;     //横向距离变化率，即dd/dt
  ...
  trajectory_analyzer->ToTrajectoryFrame(       //计算纵向误差
      VehicleStateProvider::Instance()->x(),
      VehicleStateProvider::Instance()->y(),
  ...
  TrajectoryPoint reference_point =                           // 从规划轨迹中找对应的参考点，绝对时间方法
  ...
  debug->set_station_error(reference_point.path_point().s() - s_matched);   // 位置误差, 其中s_matched = ptr_s 
  debug->set_speed_error(reference_point.v() - s_dot_matched);              // 速度误差, 其中s_dot_matched = ptr_s_dot 

  debug->set_station_reference(reference_point.path_point().s());
  debug->set_speed_reference(reference_point.v());                          //规划的速度参考值
  ...
  debug->set_preview_speed_reference(preview_point.v());                    //前一个规划的速度参考值
