...
void LatController::InitializeFilters(const ControlConf *control_conf) {
  ...
  common::LpfCoefficients(
      ts_, control_conf->lat_controller_conf().cutoff_freq(), &den, &num);    //cutoff_freq = 10
  digital_filter_.set_coefficients(den, num);//设置分子和分母
...
Status LatController::Init(const ControlConf *control_conf) {
  ...
  const int matrix_size = basic_state_size_ + preview_window_;      // 4 + 0 =4
  matrix_a_ = Matrix::Zero(basic_state_size_, basic_state_size_);   //状态矩阵A
  matrix_ad_ = Matrix::Zero(basic_state_size_, basic_state_size_);  //离散时间下的状态矩阵
  matrix_adc_ = Matrix::Zero(matrix_size, matrix_size);             //混合状态矩阵
  ...
  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);         //用于更新状态矩阵matrix_a_
  ...
  matrix_b_ = Matrix::Zero(basic_state_size_, 1);                   //控制矩阵B
  matrix_bd_ = Matrix::Zero(basic_state_size_, 1);                  //离散时间下的控制矩阵
  matrix_bdc_ = Matrix::Zero(matrix_size, 1);                       //混合控制矩阵
  matrix_b_(1, 0) = cf_ / mass_;
  matrix_b_(3, 0) = lf_ * cf_ / iz_;
  matrix_bd_ = matrix_b_ * ts_;

  matrix_state_ = Matrix::Zero(matrix_size, 1);                     //状态变量
  matrix_k_ = Matrix::Zero(1, matrix_size);                         //增益矩阵matrix_k_是时变的
  matrix_r_ = Matrix::Identity(1, 1);                               //控制权重矩阵
  matrix_q_ = Matrix::Zero(matrix_size, matrix_size);               //状态权重矩阵matrix_q_可能是时变的
  ...
  InitializeFilters(control_conf_);                               //根据配置文件初始化滤波器
...
Status LatController::ComputeControlCommand(
  ...
  // feedback = - K * state
  // 将车辆转向角由rad转换为度，再转换为度，再转换为100%
  const double steer_angle_feedback = -(matrix_k_ * matrix_state_)(0, 0) * 180 /
                                      M_PI * steer_ratio_ /
                                      steer_single_direction_max_degree_ * 100;

  const double steer_angle_feedforward = ComputeFeedForward(debug->curvature());    // 根据曲率 计算前馈

  // 限制转向角在-100.0至100.0之间
  ...
  cmd->set_steering_target(steer_angle);             //设置转弯角度，-100.0至100.0

  cmd->set_steering_rate(FLAGS_steer_angle_rate);   //设置转弯角速度
  ...
  debug->set_heading(driving_orientation_);       //车的航向
  ...
  debug->set_ref_speed(vehicle_state->linear_velocity());         //车的线速度
...
void LatController::UpdateMatrix() {
  const double v = std::max(VehicleStateProvider::Instance()->linear_velocity(),
                            minimum_speed_protection_);     //车的线速度
...
/* 在control.conf文件中use_state_exact_match = ture 调用这个函数
 * x 坐标
 * y 坐标
 * theta 航向
 * linear_v 线速度
 * angular_v 角速度
 * trajectory_analyzer 规划路径
 * debug 调试信息
*/
