...
#include "modules/canbus/vehicle/yema/yema_controller.h"
...
ErrorCode YeMaController::Init(
    const VehicleParameter &params,
    CanSender<::apollo::canbus::ChassisDetail> *const can_sender,
    MessageManager<::apollo::canbus::ChassisDetail> *const message_manager) {
  if (is_initialized_) {
    AINFO << "YeMaController has already been initiated.";
...
bool YeMaController::Start() {
  if (!is_initialized_) {
    AERROR << "YeMaController has NOT been initiated.";
    return false;
  }
...
void YeMaController::Stop() {
  if (!is_initialized_) {
    AERROR << "YeMaController stops or starts improperly!";
    return;
  }
  ...
  if (thread_ != nullptr && thread_->joinable()) {
    thread_->join();
    thread_.reset();
    AINFO << "YeMaController stopped.";
  }
}

Chassis YeMaController::chassis() {
  chassis_.Clear();
  ...
  // 19, yema wiper is too complicated
  // 24
...
void YeMaController::Emergency() {
  set_driving_mode(Chassis::EMERGENCY_MODE);
  ResetProtocol();
  set_chassis_error_code(Chassis::CHASSIS_ERROR);
}

ErrorCode YeMaController::EnableAutoMode() {
...
ErrorCode YeMaController::DisableAutoMode() {
  ResetProtocol();
  can_sender_->Update();
...
ErrorCode YeMaController::EnableSteeringOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
ErrorCode YeMaController::EnableSpeedOnlyMode() {
  if (driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
...
void YeMaController::Gear(Chassis::GearPosition ref_gear_position) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_SPEED_ONLY)) {
...
// -> pedal
void YeMaController::Brake(double pedal) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_SPEED_ONLY)) {
...
// gas:0.00~99.99 unit:%
void YeMaController::Throttle(double pedal) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_SPEED_ONLY)) {
    AINFO << "The current drive mode does not need to set acceleration.";
    return;
...
// yema default, -470 ~ 470, left:+, right:-
// need to be compatible with control module, so reverse
// steering with old angle speed
// angle:-99.99~0.00~99.99, unit:%, left:-, right:+
void YeMaController::Steer(double angle) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_STEER_ONLY)) {
...
void YeMaController::Steer(double angle, double angle_spd) {
  if (!(driving_mode() == Chassis::COMPLETE_AUTO_DRIVE ||
        driving_mode() == Chassis::AUTO_STEER_ONLY)) {
    AINFO << "The current driving mode does not need to set steer.";
    return;
...
void YeMaController::SetEpbBreak(const ControlCommand &command) {
...
void YeMaController::SetBeam(const ControlCommand &command) {
...
void YeMaController::SetHorn(const ControlCommand &command) {
...
void YeMaController::SetTurningSignal(const ControlCommand &command) {
  // Set Turn Signal
...
void YeMaController::ResetProtocol() {
  message_manager_->ResetSendMessages();
}

bool YeMaController::CheckChassisError() {
  // steer fault
  ChassisDetail chassis_detail;
  message_manager_->GetSensorData(&chassis_detail);
...
void YeMaController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR_EVERY(100)
          << "Too much time consumption in YeMaController looping process:"
          << elapsed.count();
      start = end;
    }
  }
...
bool YeMaController::CheckResponse(const int32_t flags, bool need_wait) {
...
void YeMaController::set_chassis_error_mask(const int32_t mask) {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  chassis_error_mask_ = mask;
}

int32_t YeMaController::chassis_error_mask() {
  std::lock_guard<std::mutex> lock(chassis_mask_mutex_);
  return chassis_error_mask_;
}

Chassis::ErrorCode YeMaController::chassis_error_code() {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  return chassis_error_code_;
}

void YeMaController::set_chassis_error_code(
    const Chassis::ErrorCode &error_code) {
  std::lock_guard<std::mutex> lock(chassis_error_code_mutex_);
  chassis_error_code_ = error_code;
}

bool YeMaController::CheckSafetyError(
